...
class KeychainItemSpec:
    ...
    def it_initialises_properties(self):
        eq_(self.keychain_item.key_id, "98EB2E946008403280A3A8D9261018A4")

    def it_decrypts_itself_with_decryption_key(self):
        result = self.keychain_item.decrypt(self.decryption_key)
        eq_(result['fields'][0]['name'], 'Username')
        eq_(result['fields'][0]['value'], 'somedifferentusername')
        eq_(result['fields'][0]['designation'], 'username')
        eq_(result['fields'][1]['name'], 'Password')
        eq_(result['fields'][1]['value'], 'password123')
        eq_(result['fields'][1]['designation'], 'password')
